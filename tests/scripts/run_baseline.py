#!/usr/bin/env python3
"""Run MAS evaluation baseline for tests and regression fixtures.

This script executes the full multi-agent evaluation pipeline on a given text
input. It is kept under tests/ because it primarily supports mock baseline
validation, golden snapshot generation, and regression fixtures.
"""
...
# Ensure project root is on sys.path when run as `python tests/scripts/run_baseline.py`
    ...
    if provider == "real":
        ...
        if explanation_override_dir.exists():
            for override_path in sorted(explanation_override_dir.glob("*.yaml")):
                ...
                prompt_templates[f"explanation_override_{dim_id}"] = loader.load_with_override(
                    "explanation",
                    dim_id,
