...
def test_count_consecutive_no_improvement_for_same_unit(tmp_path: Path) -> None:
    ...
    assert log.count_consecutive_no_improvement("scoring.calibration_notes.ideas_content") == 2
...
def test_mark_forbidden_unit_persists(tmp_path: Path) -> None:
    ...
    log.mark_forbidden("scoring.calibration_notes.voice", "qwk dropped > 0.03")
    assert log.is_forbidden_unit("scoring.calibration_notes.voice")

    reloaded = ExperimentLog.load(log_path)
    assert reloaded.is_forbidden_unit("scoring.calibration_notes.voice")
    meta = reloaded.forbidden_units["scoring.calibration_notes.voice"]
