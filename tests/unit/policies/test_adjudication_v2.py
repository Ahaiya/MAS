...
def test_identical_scores_never_trigger() -> None:
    ...
    chains_b = [_chain("rater_2", "A4-1", 3), _chain("rater_2", "A4-2", 4)]
...
def test_isolated_diff_of_one_does_not_trigger() -> None:
    ...
    chains_a = [_chain("rater_1", "A4-1", 3), _chain("rater_1", "A4-2", 4)]
    chains_b = [_chain("rater_2", "A4-1", 4), _chain("rater_2", "A4-2", 4)]

    assert needs_adjudication(chains_a, chains_b, _policy()) == set()


def test_score_distance_greater_than_one_triggers() -> None:
    chains_a = [_chain("rater_1", "A4-1", 2), _chain("rater_1", "A4-2", 4)]
    chains_b = [_chain("rater_2", "A4-1", 4), _chain("rater_2", "A4-2", 4)]

    assert needs_adjudication(chains_a, chains_b, _policy()) == {"A4-1"}


def test_adjacent_drift_same_direction_across_two_dims_triggers() -> None:
    chains_a = [
        _chain("rater_1", "A4-1", 3),
        _chain("rater_1", "A4-2", 3),
        _chain("rater_1", "A4-3", 4),
    ]
    chains_b = [
        _chain("rater_2", "A4-1", 4),
        _chain("rater_2", "A4-2", 4),
        _chain("rater_2", "A4-3", 4),
    ]

    assert needs_adjudication(chains_a, chains_b, _policy()) == {"A4-1", "A4-2"}


def test_adjacent_drift_opposite_directions_does_not_trigger() -> None:
    """两维分歧方向相反时不构成"同向"漂移，不触发。"""
    chains_a = [_chain("rater_1", "A4-1", 3), _chain("rater_1", "A4-2", 4)]
    chains_b = [_chain("rater_2", "A4-1", 4), _chain("rater_2", "A4-2", 3)]
...
def test_gap_above_threshold_triggers() -> None:
    chains_a = [_chain("rater_1", "A4-1", 1)]
    chains_b = [_chain("rater_2", "A4-1", 4)]

    assert needs_adjudication(chains_a, chains_b, _policy(score_gap_threshold=2)) == {"A4-1"}


def test_drift_min_dimensions_zero_never_triggers() -> None:
    """阈值为 0 时同向漂移规则整体关闭，不会把所有相邻分歧都拉进来。"""
    chains_a = [_chain("rater_1", "A4-1", 3), _chain("rater_1", "A4-2", 3)]
    chains_b = [_chain("rater_2", "A4-1", 4), _chain("rater_2", "A4-2", 4)]
...
def test_gap_and_drift_results_are_unioned() -> None:
    ...
    chains_a = [
        _chain("rater_1", "A4-1", 1),
        _chain("rater_1", "A4-2", 3),
        _chain("rater_1", "A4-3", 3),
    ]
    chains_b = [
        _chain("rater_2", "A4-1", 4),
        _chain("rater_2", "A4-2", 4),
    ...
    assert needs_adjudication(chains_a, chains_b, _policy()) == {"A4-1", "A4-2", "A4-3"}
