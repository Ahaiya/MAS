...
def test_mock_mode_produces_valid_document():
    request = EvaluationRequest(
        raw_text="A first sentence. A second sentence.",
        bundle_ref="bundle://test/v1",
        request_id="req-chunker-001",
    )
    _, doc = chunker.run(request, provider=None, template=None)
    assert doc.document_id
    assert len(doc.text_units) > 0
    assert all(u.chunk_method == "rule" for u in doc.text_units)
...
def test_llm_json_parse_failure_falls_back_to_rules():
    request = EvaluationRequest(
    ...
    assert len(doc.text_units) > 0
    assert all(u.chunk_method == "rule" for u in doc.text_units)
    assert doc.document_metadata.get("chunking_fallback_reason") == "semantic_chunking_failed"
