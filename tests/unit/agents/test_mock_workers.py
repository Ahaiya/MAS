...
- deterministic_scorer: DimensionObservation + RubricSnapshot + rater_id -> ScoreHypothesis
- deterministic_consistency_checker: List[ScoreHypothesis] + PolicySnapshot -> List[ConflictRecord]
- deterministic_adjudicator: (conflicts, hypotheses, policy) -> (AdjudicationRecords, FinalDimensionDecisions)
- feedback: (decisions, observations, spans, rubric, policy) -> Dict[str, Any]
...
def coverage_plans(document, rubric):
    return coverage.run(document, rubric)
...
def spans_dim1(plan_dim1, document):
    return deterministic_extractor.run(plan_dim1, document)


@pytest.fixture
def observation_dim1(spans_dim1, plan_dim1):
    return observer.run(spans_dim1, plan_dim1)
...
class TestConfigResolver:
    def test_returns_resolved_artifact_bundle(self):
        ...
        result = config_resolver.run(bundle_path)
...
class TestCoverage:
    ...
    def test_deterministic(self, document, rubric):
        plans1 = coverage.run(document, rubric)
        plans2 = coverage.run(document, rubric)
...
class TestMockExtractor:
    def test_returns_list_of_evidence_spans(self, plan_dim1, document):
        spans = deterministic_extractor.run(plan_dim1, document)
    ...
    def test_deterministic(self, plan_dim1, document):
        spans1 = deterministic_extractor.run(plan_dim1, document)
        spans2 = deterministic_extractor.run(plan_dim1, document)
    ...
    def test_different_plans_produce_different_spans(self, coverage_plans, document):
        ...
        spans_a = deterministic_extractor.run(plan_a, document)
        spans_b = deterministic_extractor.run(plan_b, document)
...
class TestObserver:
    def test_returns_dimension_observation(self, spans_dim1, plan_dim1):
        obs = observer.run(spans_dim1, plan_dim1)
...
class TestMockScorer:
    def test_returns_score_hypothesis(self, observation_dim1, rubric):
        hyp = deterministic_scorer.run(observation_dim1, rubric, "rater_1")
    ...
    def test_different_rater_ids_produce_different_hypothesis_ids(self, observation_dim1, rubric):
        hyp1 = deterministic_scorer.run(observation_dim1, rubric, "rater_1")
        hyp2 = deterministic_scorer.run(observation_dim1, rubric, "rater_2")
        assert hyp1.hypothesis_id != hyp2.hypothesis_id

    def test_deterministic(self, observation_dim1, rubric):
        hyp1 = deterministic_scorer.run(observation_dim1, rubric, "rater_1")
        hyp2 = deterministic_scorer.run(observation_dim1, rubric, "rater_1")
        assert hyp1 == hyp2
...
class TestMockConsistencyChecker:
    def test_returns_list(self, policy):
        result = deterministic_consistency_checker.run([], policy)
    ...
    def test_no_conflicts_when_scores_agree(self, policy):
        ...
        conflicts = deterministic_consistency_checker.run(hyps, policy)
        assert conflicts == []
    ...
    def test_conflict_record_references_both_hypotheses(self, policy):
        ...
        conflicts = deterministic_consistency_checker.run([h1, h2], policy)
...
class TestDeterministicAdjudicator:
    ...
    def test_returns_tuple_of_two_lists(self, policy):
        result = deterministic_adjudicator.run([], [], policy)
        assert isinstance(result, tuple)
        assert len(result) == 2
    ...
    def test_no_conflict_produces_decision_for_each_dimension(self, policy):
        ...
        _, decisions = deterministic_adjudicator.run([], hyps, policy)
    ...
    def test_adjudication_record_for_each_conflict(self, policy):
        ...
        adj_records, _ = deterministic_adjudicator.run([conflict], [h1, h2], policy)
    ...
    def test_resolved_conflict_produces_final_decision(self, policy):
        ...
        h2 = _make_hypothesis("dim_001", "obs-1", "rater_2", 4)
        conflict = self._make_conflict(h1, h2)
        adj_records, decisions = deterministic_adjudicator.run([conflict], [h1, h2], policy)
    ...
    def test_final_decision_is_correct_type(self, policy):
        ...
        h2 = _make_hypothesis("dim_001", "obs-1", "rater_2", 4)
        conflict = self._make_conflict(h1, h2)
        _, decisions = deterministic_adjudicator.run([conflict], [h1, h2], policy)
        for d in decisions:
    ...
    def test_deterministic(self, policy):
        ...
        h2 = _make_hypothesis("dim_001", "obs-1", "rater_2", 4)
        conflict = self._make_conflict(h1, h2)
        r1, d1 = deterministic_adjudicator.run([conflict], [h1, h2], policy)
        r2, d2 = deterministic_adjudicator.run([conflict], [h1, h2], policy)
...
class TestFeedback:
    ...
    def test_returns_dict(self, rubric, policy):
        decisions = [self._make_decision("dim_001", 3)]
        observations = [self._make_observation("dim_001")]
        result = feedback.run(decisions, observations, [], rubric, policy)
        assert isinstance(result, dict)

    def test_has_dimensions_key(self, rubric, policy):
        decisions = [self._make_decision("dim_001", 3)]
        observations = [self._make_observation("dim_001")]
        result = feedback.run(decisions, observations, [], rubric, policy)
        assert "dimensions" in result

    def test_dimensions_keyed_by_dimension_id(self, rubric, policy):
        decisions = [
    ...
    def test_dimension_entry_has_score(self, rubric, policy):
        decisions = [self._make_decision("dim_001", 3)]
        observations = [self._make_observation("dim_001")]
        result = feedback.run(decisions, observations, [], rubric, policy)
    ...
    def test_dimension_entry_has_display_score(self, rubric, policy):
        decisions = [self._make_decision("dim_001", 3)]
        observations = [self._make_observation("dim_001")]
        result = feedback.run(decisions, observations, [], rubric, policy)
        entry = result["dimensions"]["dim_001"]
    ...
    def test_dimension_name_from_rubric_config(self, rubric, policy):
        decisions = [self._make_decision("dim_001", 3)]
        observations = [self._make_observation("dim_001")]
        result = feedback.run(decisions, observations, [], rubric, policy)
        entry = result["dimensions"]["dim_001"]
    ...
    def test_deterministic(self, rubric, policy):
        decisions = [self._make_decision("dim_001", 3)]
        observations = [self._make_observation("dim_001")]
        r1 = feedback.run(decisions, observations, [], rubric, policy)
        r2 = feedback.run(decisions, observations, [], rubric, policy)
