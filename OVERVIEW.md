...
**当前主线任务**：`maker_hackathon`（智造黑客松），评价维度 A4（用户理解与需求洞察），含三个观测点 A4-1 / A4-2 / A4-3，1–5 级量表。

---

## 二、整体架构

系统分为**内环流水线**和**外环纠偏机制**两层：
...
3. **CorrectionAgent**（`src/outer_loop/correction_agent.py`）：读取当前 `task_context.yaml` + 修正事件 → 调用 LLM → 生成更新后的 YAML → 通过 `ConfigPatcher` 写入并快照
4. **修正信号映射**：
...
| `scripts/outer_loop.py` | 外环管理命令（run/status/rollback） |
...
| `src/outer_loop/optimization/config_patcher.py` | ConfigPatcher（白名单 + 快照 + patch） |
