...
命名与 ticket 描述的"feedback.py"不同——`src/agents/feedback.py` 已被 v1 占用
（旧文件本票不改，留到 09 删除），故本模块另起 report.py，对齐 spec.md 里
"reconcile →[adjudicate]→ feedback" 流水线图中 feedback 这一阶段的职责。"""
...
def build_feedback_report(
    ...
    for decision in sorted(decisions, key=lambda d: d.dimension_id):
        dimension = rubric.get_dimension(decision.dimension_id)
...
def build_rater_chains_report(
    ...
    all_chains = sorted(list(chains_a) + list(chains_b), key=lambda c: (c.dimension_id, c.rater_id))
    return {
        "chains": [c.to_dict() for c in all_chains],
        "final_decisions": [d.to_dict() for d in sorted(decisions, key=lambda d: d.dimension_id)],
    }
