...
| `compute_coverage_metrics.py` | **覆盖率诊断工具**。读取 `evidence_spans.json` 与 `observations.json`，输出 coverage recall / precision / chunk boundary 指标 |
| `validate_config.py` | **配置校验工具**。将 bundle YAML 经过完整 resolver/compiler 流程，报告版本闭包、schema 校验和 freeze hash |

## tests/scripts/ 目录

存放**仅供测试、基线回归和 golden snapshot 验证**使用的命令行脚本，不属于真实运行入口。

| 文件 | 作用 |
|------|------|
| `run_baseline.py` | **基线运行脚本**。执行完整多智能体评估流水线，主要供 mock baseline / golden snapshot 相关测试调用 |
| `accept_baseline.py` | **基线接受脚本**。对 manifest 中所有样本跑完整基线流水线，验证每个运行目录，并可选检查 golden snapshot |
| `compare_snapshot.py` | **快照对比工具**。将当前运行目录与 golden snapshot 对比，检测状态、节点序列、维度得分、裁决冲突数等维度是否退化 |
| `replay_run.py` | **回放工具**。从 golden snapshot 读取配置并重跑流水线，配合 `compare_snapshot.py` 做回归检测 |
| `validate_run.py` | **运行目录校验工具**。检查测试产物是否合法：必需文件存在、run_trace schema、所有节点状态为 success、feedback 结构完整等 |
